...
using E_Commerce_Web.Service.Momo.Response;
using System.Diagnostics;
using Org.BouncyCastle.Asn1.Ocsp;

namespace E_Commerce_Web.Service.Momo.Request
{
    public class MomoOneTimePaymentRequest
    {
        ...
        public (bool, string) GetLink(string paymentUrl)
        {
            ...
            if (createPaymentLinkRes.IsSuccessStatusCode)
            {
                ...
                {
                    Debug.WriteLine("CODE" + responseData.resultCode);
                    return (false, responseData.message);
                }
            }
            else
            {
                Debug.WriteLine("CODE 2" + createPaymentLinkRes.Content);
